...
export function SceneViewer({ sceneUrl, progress, title }: SceneViewerProps) {
  ...
  return (
    <section className="relative h-full overflow-hidden bg-[#c7d0cc]" aria-label="Interactive 3D room">
      <Canvas shadows camera={{ position: [2.58, 1.55, 4.47], fov: 48 }}>
        ...
        {sceneUrl && <Suspense fallback={null}><Room key={sceneUrl} sceneUrl={sceneUrl} /></Suspense>}
        {mode === "orbit" ? (
      ...
      {sceneUrl && mode === "walk" && !progress && <div className="pointer-events-none absolute inset-x-0 top-24 mx-auto w-fit rounded-full bg-[#17221c]/80 px-4 py-2 text-xs text-white backdrop-blur">Click the room, then use WASD · Esc releases the cursor</div>}
