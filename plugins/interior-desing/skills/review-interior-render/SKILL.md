...
| Human use and reach | Inspect prominent handles and controls from the user's standing position. For ordinary hinged wall-cabinet doors, verify lower opening-side pull placement, grip height above the floor, intervening counter depth, hand clearance and opening sweep using the kitchen-construction reference. Record the measured geometry or diagnostic view. Hardware that the intended user cannot reach or operate with the intended access method is a design failure even in an otherwise convincing photograph. Distinguish intentional high storage and alternative opening mechanisms. |
