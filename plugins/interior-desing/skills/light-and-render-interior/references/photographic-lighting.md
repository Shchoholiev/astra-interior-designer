...
For a photographic interior, verify that apparent ceiling and wall regions are actual visible surfaces rather than the world background. A shell that affects diffuse rays but has camera visibility disabled can preserve bounce while leaving a flat gray void, unsupported pendant canopy, or exposed cutaway edge in the delivered image. Inspect camera-ray visibility separately from render visibility, and compare a controlled preview with the relevant surface visible. Keep intentional cutaways for diagrammatic views; do not carry their blanket camera-hiding rules into a photographic render. Preserve the selected camera and room dimensions, and diagnose any occlusion individually instead of hiding the complete ceiling.
