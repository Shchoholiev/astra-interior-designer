...
Deliver the native `.blend`, final image, useful before/after image links, and portable texture/asset dependencies with provenance. A viewer export is optional and does not replace the native master. Include the final visual assessment from `review-interior-render`: a verdict against the brief, the status and visible evidence for dominant findings, and the constraint behind each unresolved issue. Complete in-scope corrections and inspect the resulting pass before calling the refinement finished. Only call an image rendered after the job completes and its output has been inspected; if a runtime failure prevents that, identify the last valid checkpoint and remaining action.
